  ...
  const fetchData = async () => {
    try {
      ...
      const [dayResponse, monthResponse, yearResponse] = await Promise.all([
        axios.get('http://localhost:3333/api/relatorios/vendas-dia'),
        axios.get('http://localhost:3333/api/relatorios/vendas-mes'),
        axios.get('http://localhost:3333/api/relatorios/vendas-ano')
      ]);
