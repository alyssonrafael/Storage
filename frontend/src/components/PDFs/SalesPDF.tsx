  ...
  <Document>
    <Page size="A4" style={styles.page}>
      <View style={styles.section}>
        ...
        Forma de Pagamento: {data.formaDePagamento}
        </Text>
        ...
        {data.produtos.map((produto) => (
          <View key={produto.id} style={styles.item}>
            <Text style={styles.item}>{produto.produto.nome}</Text>
            <Text style={styles.item}>Quantidade: {produto.quantidade}</Text>
            <Text style={styles.item}>
              Preço Unitário: R${produto.produto.preco.toFixed(2)}
            </Text>
          </View>
        ))}
        ...
        <Text style={styles.item}>
            Total R${(data.total + data.desconto).toFixed(2)}
          </Text>
        <Text style={styles.item}>Desconto: R${data.desconto}</Text>
        <Text style={styles.item}>
        Subtotal da Venda: R${data.total.toFixed(2)}
        </Text>
      </View>
    </Page>
  </Document>
